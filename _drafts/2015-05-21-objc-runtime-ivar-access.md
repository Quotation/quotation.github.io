...
看代码，考虑Objective-C程序里最常见的操作之一——访问成员变量。
...
我一直到昨天中午之前也没想到这些，但当明白真相后，发现还没有文章真正讲清楚过Objective-C的类成员变量（ivar，instance variables，类实例量变），于是有必要做个深度剖析。
...
直到昨天一个朋友问我runtime的问题，我了解到“non-fragile instance variables”的概念。
...
这是苹果官方文档[Objective-C Runtime Programming Guide](https://developer.apple.com/library/mac/documentation/Cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtVersionsPlatforms.html#//apple_ref/doc/uid/TP40008048-CH106-SW1)上的一段话，意思是在“modern runtime”里，如果你修改了父类的成员变量布局（比如增加成员变量），子类不需要重新编译。这是一个巨大的改动，在文档中当做“modern runtime”最重要的修改点被提出来。

这个特性的重大意义在于，Objective-C的库从此具有了*“二进制兼容性”*。举例来说，你在项目里用了第三方提供的静态库SDK，包含一些`.h`和一个`.a`文件。当iOS SDK的版本从6升到了7，又从7升到了8时，你都不需要更新这个SDK。虽然iOS SDK版本升级时，苹果在UIView等基类中加入了更多的成员变量，很多基类都做了大的改动，但是静态发布的SDK不需要重新编译还能正常使用。
...
2) 当苹果发布新版本OSX SDK后，NSObject增加了两个成员变量。如果没有Non Fragile ivars特性，我们的代码将无法正常运行了，因为MyObject类成员变量布局在编译时已经确定，有两个成员变量和基类的内存区域重叠了。此时，我们只能重新编译代码，程序才能在新版本系统上运行。如果更悲催一点，MyObject类是来自第三方提供的二进制库，我们就只能眼巴巴等着库作者更新版本了。
...
基本原理就是这样。听起来不新鲜，很多编程语言都能做到，比如Java、C#，都有二进制兼容性。可是Objective-C毕竟不是“那么”动态的语言，Objective-C代码编译后是真正的native二进制，不是byte code。Objective-C程序也不是运行在VM上，底下只有个很小的runtime。
...
首先去[http://opensource.apple.com/](http://opensource.apple.com/)下载runtime源码，在OSX目录里，当前最新版本是objc4-646.tar.gz。解压后，打开`objc-runtime-new.h`和`objc-runtime-new.mm`两个文件。
...
我们看到了重要的`offset`字段，这里一定是记录着这个成员变量在对象中的偏移位置喽。也就是说，runtime在发现基类大小变化时，是通过修改`offset`，来更新子类成员变量的偏移值。那Objective-C中获取对象的第N个成员变量偏移位置就需要这样一长串代码：

```
*((&obj->isa.cls->data()->ro->ivars->first)[N]->offset)
```

这么多次寻址，看起来很可怕。每个成员变量都这样访问的话，性能一定无法接受。看看编译器到底是如何实现的吧，我们祭出LLVM。
...
编译结果`test.ll`就是LLVM IR代码。用Sublime打开，安装LLVM语法高亮插件。可以看到IR格式比较繁琐，比汇编简单，比C复杂。这里就不写出IL的分析过程了，直接说结论。

编译后的`obj->myInt = 42`调用对应于如下的C语言代码。
...
两条CPU指令搞定。第一条取`g_ivar_MyClass_myInt`的值，第二条寻址并赋值。根本不需要一长串的指针调用。LLVM为每个类的每个成员变量都分配了一个全局变量，用于存储该成员变量的偏移值。

这也就是为什么`ivar_t.offset`用*int指针*来存储偏移值，而不是直接放一个int的原因。通过这样设计，真正存放偏移值的地址是固定不变的，在编译时就确定了下来。

这就是Objective-C类成员变量的寻址方式。编译器通过这种方式，达到了灵活性和执行效率的完美平衡！

### Non Fragile实现

有了这种灵活而高效的寻址方式，那runtime是在什么时候调整成员变量偏移值的呢？从IR中可以看到，在编译时，LLVM计算出基类NSError对象的大小为40字节，然后记录在MyClass的类定义中，如下是对应的C代码。
...
现在假如苹果发布了OSX 11，NSError类大小变为48字节。当我们的程序启动后，runtime加载MyClass类定义的时候，发现基类的真实大小和MyClass的instanceStart不相符，得知基类的大小发生了改变。于是runtime遍历MyClass的所有成员变量定义，将`offset`指向的值增加8。具体的实现代码在`objc-runtime-new.mm`的`moveIvars()`函数中。

并且，`instanceSize`也增加了8。这样在runtime创建MyClass对象的时候，能分配出正确大小的内存块。
...
这个问题的答案与Non Fragile ivars无关，但既然此文是讨论类成员变量的，因此一并回答。很多人在学到Category时都会有疑问，为什么可以用Category给类增加方法和属性，但是不能增加成员变量？

在[Objective-C提供的runtime函数](https://developer.apple.com/library/mac/documentation/Cocoa/Reference/ObjCRuntimeRef/index.html)中，确实见到了一个`class_addIvar`函数，但是文档中特别说明：

> This function may only be called after objc_allocateClassPair and before objc_registerClassPair. Adding an instance variable to an existing class is not supported.

意思是，这个函数只能在动态构建一个类的过程中调用，一旦完成类定义，调用了`objc_registerClassPair`之后，就不能再添加成员变量了。
...
MFC动态库常遇到的问题 / C++开发常遇到的版本问题，静态编译所有依赖库 / 通过接口设计规避ABI兼容
